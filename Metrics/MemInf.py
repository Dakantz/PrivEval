...
def calculate_metric(args=None, _real_data=None, _synthetic=None, num_eval_iter=5):
    ...
    try:
        ...
        for i in range(num_eval_iter):
            ...
            print(f"MIR evaluation iteration {i+1}/{num_eval_iter}")
            try:
                D_train, D_test, L_train, L_test = train_test_split(
                    D_encoded, L, test_size=0.3, random_state=i, stratify=L
                )
            except ValueError:
                # If stratification fails, do simple split
                D_train, D_test, L_train, L_test = train_test_split(
                    D_encoded, L, test_size=0.3, random_state=i
                )

            # Step 4: Train LightGBM classifier
            # f_θ = LightGBM(D_train)
            cls = LGBMClassifier(verbosity=-1, random_state=42, n_jobs=-1, device="gpu")
